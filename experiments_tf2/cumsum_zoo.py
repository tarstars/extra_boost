...
def hreblock(arr, isplus=False, digit=0):
    ...
    # arr2x = arr2
    if isplus:
        arr2 = tf.cond(pred=tf.equal(tf.shape(input=arr1)[1], 0), true_fn=lambda: arr2, false_fn=lambda: arr2 + arr1[:, -1:])
...
def vreblock(arr, isplus=False, digit=0):
    ...
    arr2 = arr[:, half1:]
    if isplus:
        arr2 = tf.cond(pred=tf.equal(tf.shape(input=arr1)[0], 0), true_fn=lambda: arr2, false_fn=lambda: arr2 + arr1[-1:, :])
...
def mycumsum2(arr, axis, name):
    m = 100000.0
    return tf.cast(tf.cumsum(tf.cast(arr * m, dtype=tf.int64), axis=axis, name=name), dtype=arr.dtype)/tf.cast(m, dtype=arr.dtype)
