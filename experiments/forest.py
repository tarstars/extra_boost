...
def split_ematrix(ematrix, depth, params, split_maker, sess=None):
    ...
    if ematrix.gax is not None:
        ...
        split_info = split_maker.split(
            ...
            ax=ematrix.gax,
            params=params,
            # profile_file=("profile_cumsum3.json" if depth == 1 else None),
            sess=sess,
        )
    ...
    left_ematrix = EMatrix(
        features=getslice(features, cond_left, split_maker.reduce_axis),
    ...
    right_ematrix = EMatrix(
        features=getslice(features, cond_right, split_maker.reduce_axis),
...
def build_tree_helper(params, info, parent, split_maker):
    ...
    left_info, right_info, split_info = split_ematrix(
        info["ematrix"], node.depth, params, split_maker=split_maker, sess=info["sess"]
    )
    ...
    node.left = build_tree_helper(
        params, left_info, parent=node, split_maker=split_maker
    )
    node.right = build_tree_helper(
        params, right_info, parent=node, split_maker=split_maker
    )
    return node
...
def build_tree(params, ematrix, split_maker, sess=None):
    ...
    if ematrix.splitgax and ematrix.gax is None:
        ematrix.gax = split_maker.make_gax(
            ematrix.features, axis=split_maker.reduce_axis
        )
